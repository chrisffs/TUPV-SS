...
def parse_answer_key(answer_key_list):
    result_dict = {}
    
    for i, val in enumerate(answer_key_list):
        val = int(val)
...
def save_images(images, titles, folder_path="outputs"):
...
def main():
    ...
    # python omrscanner.py "test.png" "3 3 3 3 3 2 1 3 2 3 3 2 3 3 2 2 2 4 1 1" 4

    image = load_image(sys.argv[1])
    image_name = sys.argv[4]
    
    answer_key = sys.argv[2]
    answers_list = [int(val) if val.isdigit() else val for val in answer_key.split()]
    ANSWER_KEY = parse_answer_key(answers_list)
    # print(ANSWER_KEY)
