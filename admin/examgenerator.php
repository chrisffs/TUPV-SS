         ...
         <div>
            ...
            <table id="examGenListTable" class="pt-3 mb-3 w-full text-sm text-left text-gray-500 dark:text-gray-400">
               ...
               <?php foreach ($data1 as $row): ?>
                  ...
                  <tr onclick="redirectToExamPrint('<?php echo $row['uniquecode']; ?>', '<?php echo $row['Term']; ?>', '<?php echo $row['Subj']; ?>', '<?php echo $row['Semester']; ?>', '<?php echo $row['exam_sets']; ?>', '<?php echo $row['uploaderId']; ?>')" class="bg-white border-b dark:bg-gray-800 cursor-pointer dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
...
function redirectToExamPrint(uniquecode, term, subj, semester, sets, id) {
   // Redirect to examprint.php with the data
   window.location.href = `./Examprint.php?uniquecode=${uniquecode}&Term=${term}&Subj=${subj}&Semester=${semester}&Sets=${sets}&ID=${id}`;
}
</script>
